...
use crate::model::{
    AuthAction, AuthStatus, CalendarEvent, Conversation, Message, MessageAction, Person,
    SeedResult, Team, Thread, TokenInfo,
};
...
impl<T: DisplayOutput> DisplayOutput for Vec<T> {
    fn display_output(&self) -> String {
        self.iter()
            .map(DisplayOutput::display_output)
            .collect::<Vec<_>>()
            .join("\n")
    }
}
...
impl DisplayOutput for Message {
    fn display_output(&self) -> String {
        ...
        let when = self
            .compose_time
            .as_deref()
            .or(self.original_arrival_time.as_deref())
            .unwrap_or("");
    ...
    fn display_output(&self) -> String {
        match &self.emoji {
            Some(emoji) => {
                format!(
                    "{} {emoji} on message {} in {}",
                    self.action, self.message_id, self.conversation
                )
            }
            None => format!(
                "{} message {} in {}",
...
impl DisplayOutput for TokenInfo {
    fn display_output(&self) -> String {
        ...
        let ttl = self
            .expires_in_min
            .map_or_else(|| "n/a".to_owned(), |min| format!("{min} min"));
...
impl DisplayOutput for SeedResult {
    fn display_output(&self) -> String {
        ...
        let mut lines = vec![
            format!(
                "Seeded {} ({} token) for {who}",
                self.target, self.token_type
            ),
...
mod tests {
    ...
    fn message_list_is_earliest_first_in_json_and_human() {
        ...
        assert!(
            ja < jb && jb < jc,
            "JSON order must be earliest-first: {json}"
        );
        ...
        assert!(
            ta < tb && tb < tc,
            "human order must be earliest-first: {text}"
        );
    }
}
